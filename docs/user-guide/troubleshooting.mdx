...
See [Sampling](../user-guide/sampling.mdx) for how to change which rank(s) export.
...
On non-exporting ranks, `setup_telemetry` sets the enabled group set to empty regardless of config. Nothing in this rank will export, by design. See [Sampling](../user-guide/sampling.mdx).
...
Switch from `per_step` or `all` to `default`. Spans in the hot loop (microbatch, communication) are the likely cost. See [Span Groups](../user-guide/span-groups.mdx) and [Sampling](../user-guide/sampling.mdx) for how to dial this in.
