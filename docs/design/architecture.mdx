...
| `handle.py` | `setup_telemetry` entry point, `TelemetryHandle` |
...
```
setup_telemetry(config, resource_attributes)
  │
  ...
  │
  ├─ if config.enabled:
  │     build_providers(config, resource_attributes, ...)
  │         ├─ imports SDK (primary SDK construction site)
  │         ├─ builds Resource with auto-detected + passed attributes
  │         ├─ resolves service.instance.id from the caller's dl.rank, if given
  │         ├─ builds TracerProvider (SeedIndependentIdGenerator)
...
**Lazy SDK imports**: `providers.py` is the primary home for `opentelemetry.sdk.*` construction (`logging_bridge.py` also imports the SDK lazily inside a function body). A process with telemetry disabled never executes `build_providers`, so it never incurs the SDK import cost. In a large job where most ranks leave `enabled` false, avoiding that import on every quiet rank adds up.
