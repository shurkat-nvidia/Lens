...
The fallback signatures must match the real API exactly: the parameter names, whether each parameter is positional or keyword-only, and its default value. It is fine for a no-op to ignore the value it is given. It is not fine for the default to differ, because then leaving the argument out means one thing with NeMo Lens installed and something else without it — and only consumers running without it are affected, so nobody notices. `lens/tests/test_fallbacks.py` compares all three for every fallback symbol and every `SpanRegistry` method.
